...
class UserController(
    ...
    fun createUserByPhone(@RequestBody request: CreateUserByPhoneRequest): ResponseEntity<ApiResponse<UserResponse>> {
    ...
    fun getMyInfo(@AuthenticationPrincipal userDetails: UserDetails): ResponseEntity<ApiResponse<UserResponse>> {
    ...
    @Operation(summary = "내 상세 정보 조회", description = "현재 로그인한 사용자의 상세 정보(소셜 연동, 관심 카테고리 포함)를 조회합니다.")
    fun getMyDetailInfo(@AuthenticationPrincipal userDetails: UserDetails): ResponseEntity<ApiResponse<UserDetailResponse>> {
        val userId = userDetails.username.toLong()
        val output = userUseCase.getUserById(userId)
    ...
    fun updateMyProfile(
        ...
        val response = mapper.toUserProfileUpdateResponse(
    ...
    fun sendPhoneVerificationCode(@RequestBody request: SendPhoneVerificationRequest): ResponseEntity<ApiResponse<PhoneVerificationResponse>> {
        val result = userUseCase.sendPhoneVerificationCode(request.phoneNumber, request.verificationType)
    ...
    fun verifyPhoneCode(@RequestBody request: VerifyPhoneCodeRequest): ResponseEntity<ApiResponse<PhoneVerificationResultResponse>> {
        val result = userUseCase.verifyPhoneCode(request.phoneNumber, request.verificationCode, request.verificationType)
        val response = mapper.toPhoneVerificationResultResponse(
            success = result.success,
            message = result.message,
    ...
    fun connectSocialAccount(
        ...
        val response = mapper.toSocialConnectionResultResponse(
            success = result.success,
            message = result.message,
    ...
    fun getMyStats(@AuthenticationPrincipal userDetails: UserDetails): ResponseEntity<ApiResponse<UserStatsResponse>> {
        val userId = userDetails.username.toLong()
    ...
    fun setupTwoFactorAuth(@AuthenticationPrincipal userDetails: UserDetails): ResponseEntity<ApiResponse<TwoFactorSetupResponse>> {
        val userId = userDetails.username.toLong()
    ...
    fun verifyTwoFactorAuth(
        ...
        val userId = userDetails.username.toLong()
        val result = userUseCase.verifyTwoFactorAuth(userId, request.code)
    ...
    fun deleteMyAccount(@AuthenticationPrincipal userDetails: UserDetails): ResponseEntity<ApiResponse<Map<String, String>>> {
        val userId = userDetails.username.toLong()
    ...
    fun getInterestedCategories(@AuthenticationPrincipal userDetails: UserDetails): ResponseEntity<ApiResponse<List<CategoryResponse>>> {
        val userId = userDetails.username.toLong()

...
public class ProductImageWebMapper {
  ...
  public List<UpdateProductImageInput> toUploadImageInputs(
      Long productId, List<MultipartFile> files, List<ProductImageMetaRequest> metas) {
    return IntStream.range(0, files.size())
        .mapToObj(i -> toUploadImageInput(productId, files.get(i), metas.get(i)))
        .toList();
  }

  private UpdateProductImageInput toUploadImageInput(
      Long productId, MultipartFile file, ProductImageMetaRequest meta) {
    try {

...
interface ProductItemRepository : JpaRepository<ProductItemEntity, Long> {
    ...
    // 카탈로그 ID로 아이템 검색
    fun findByProductCatalogId(catalogId: Long, pageable: Pageable): Page<ProductItemEntity>
    ...
    // 제품 상태(NEW/USED)로 검색
    fun findByCondition(condition: ProductConditionEntity, pageable: Pageable): Page<ProductItemEntity>
    ...
    @Query(
        ...
        JOIN ProductCatalogEntity pc ON pi.productCatalogId = pc.id
        WHERE (:catalogId IS NULL OR pi.productCatalogId = :catalogId)
        AND (:sellerId IS NULL OR pi.sellerId = :sellerId)
    ...
    fun searchItems(
        ...
        @Param("condition") condition: ProductConditionEntity?,

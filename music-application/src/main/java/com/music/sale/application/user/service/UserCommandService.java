            ...
            .orElseThrow(
                () ->
                    new BusinessException(
                        UserErrorCode.USER_NOT_FOUND, "사용자를 찾을 수 없습니다: " + userId));
  ...
  public void agreeToTerms(AgreeTermsInput input, Long currentUserId) {
    ...
    userQueryPort
        ...
        .orElseThrow(
            () ->
                new BusinessException(
                    UserErrorCode.USER_NOT_FOUND, "사용자를 찾을 수 없습니다: " + input.userId()));

    UserTerms userTerms =
        UserTerms.of(input.userId(), input.title(), input.version(), input.isAgreed());

...
/** LikePersistenceAdapter 통합 테스트 실제 DB(H2)에 연결하여 CRUD 동작 검증 */
@SpringBootTest
@Transactional
@ActiveProfiles("test")
...
class LikePersistenceAdapterIntegrationTest {
  ...
  @Autowired private LikeCommandPersistenceAdapter commandAdapter;

  @Autowired private LikeQueryPersistenceAdapter queryAdapter;
  ...
  void findByUserIdAndType_Success() {
    ...
    assertThat(result.getContent()).extracting(Like::userId).containsOnly(1L);
  }
